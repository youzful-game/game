...
export default class LFBGameState extends Phaser.Scene {
  ...
  public initAudio(): void {
    ...
    this.sfxMusic = this.sound.add("music_LFB");
    this.sfxMusic.play();
  ...
  public endLevel(): void {
    if (this.sfxMusic instanceof Phaser.Sound.WebAudioSound) {
      this.sfxMusic.stop();
      this.lfbWon = true;
      this.scene.start("LevelClearedState", { musicOn: this.musicOn, startup: "lfb", lfbWon: this.lfbWon, yaplaWon: this.yaplaWon, klsWon: this.klsWon, blankWon: this.blankWon });
    }
  }
  ...
  public update(): void {
    ...
    if (this.lives <= 0) {
      ...
      this.lives = 3;
      if (this.sfxMusic instanceof Phaser.Sound.WebAudioSound) {
        GameAnalytics.addProgressionEvent(EGAProgressionStatus.Fail, "lfb");
