...
export default class BLANKGameState extends Phaser.Scene {
  ...
  private readonly levelHeight: number = 5;
  ...
  public initUi(): void {
    ...
    // Text
    this.levelText = this.add.text(150, 17, `LEVEL : ENJOY YOUZFUL`, {
      fill: "#92cd00",
      fontFamily: 'gotham',
      fontSize: "24px"
    });
  ...
  public initAudio(): void {
    ...
    this.sfxMusic = this.sound.add("music_BLANK");
    this.sfxMusic.play();
  ...
  public endLevel(): void {
    if (this.sfxMusic instanceof Phaser.Sound.WebAudioSound) {
      this.sfxMusic.stop();
      this.blankWon = true;
      this.scene.start("LevelClearedState", { musicOn: this.musicOn, startup: "blank", lfbWon: this.lfbWon, yaplaWon: this.yaplaWon, klsWon: this.klsWon, blankWon: this.blankWon });
    }
  }
  ...
  public update(): void {
    ...
    if (this.lives <= 0) {
      ...
      this.lives = 3;
      if (this.sfxMusic instanceof Phaser.Sound.WebAudioSound) {
        GameAnalytics.addProgressionEvent(EGAProgressionStatus.Fail, "blank");
    ...
    this.levelText.setText(`LEVEL : ENJOY YOUZFUL`);
  }
