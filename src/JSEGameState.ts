...
export default class JSEGameState extends Phaser.Scene {
  ...
  public initUi(): void {
    ...
    // Text
    this.levelText = this.add.text(370, 12, `level: JSE`, {
      fill: "#FFFFFF",
      fontFamily: 'gotham',
      fontSize: "16px"
    });
  ...
  public initAudio(): void {
    ...
    this.sfxMusic = this.sound.add("music_JSE");
    this.sfxMusic.play();
  ...
  public endLevel(): void {
    if (this.sfxMusic instanceof Phaser.Sound.WebAudioSound) {
      this.sfxMusic.stop();
      this.scene.start("LevelClearedState", { musicOn: this.musicOn, startup: "jse" });
    }
  }
  ...
  public update(): void {
    ...
    if (this.lives <= 0) {
      ...
      this.lives = 3;
      if (this.sfxMusic instanceof Phaser.Sound.WebAudioSound) {
        GameAnalytics.addProgressionEvent(EGAProgressionStatus.Fail, "jse");
    ...
    this.levelText.setText(`LEVEL: JSE`);
  }
