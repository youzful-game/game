...
export default class KLSGameState extends Phaser.Scene {
  ...
  private readonly levelHeight: number = 3;
  ...
  public initUi(): void {
    ...
    // Text
    this.levelText = this.add.text(130, 17, `LEVEL : YOUZFUL CAMPAGNE`, {
      fill: "#92cd00",
      fontFamily: 'gotham',
      fontSize: "24px"
    });
  ...
  public initAudio(): void {
    ...
    this.sfxMusic = this.sound.add("music_KLS");
    this.sfxMusic.play();
  ...
  public endLevel(): void {
    if (this.sfxMusic instanceof Phaser.Sound.WebAudioSound) {
      this.sfxMusic.stop();
      this.klsWon = true;
      this.scene.start("LevelClearedState", { musicOn: this.musicOn, startup: "kls", lfbWon: this.lfbWon, yaplaWon: this.yaplaWon, klsWon: this.klsWon, blankWon: this.blankWon });
    }
  }
  ...
  public update(): void {
    ...
    if (this.lives <= 0) {
      ...
      this.lives = 3;
      if (this.sfxMusic instanceof Phaser.Sound.WebAudioSound) {
        GameAnalytics.addProgressionEvent(EGAProgressionStatus.Fail, "kls");
    ...
    this.levelText.setText(`LEVEL : YOUZFUL CAMPAGNE`);
  }
